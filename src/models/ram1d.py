...
class RAM_ResidualBlock(nn.Module):
    def __init__(self, input_channels, output_channels, stride=1):
        super(RAM_ResidualBlock, self).__init__()
        self.input_channels = input_channels
        self.output_channels = output_channels
        self.stride = stride
        self.bn1 = nn.BatchNorm1d(input_channels)
        self.relu = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv1d(input_channels, output_channels/4, 1, 1, bias = False)
        self.bn2 = nn.BatchNorm1d(output_channels/4)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv1d(output_channels/4, output_channels/4, 3, stride, padding = 1, bias = False)
        self.bn3 = nn.BatchNorm1d(output_channels/4)
        self.relu = nn.ReLU(inplace=True)
        self.conv3 = nn.Conv1d(output_channels/4, output_channels, 1, 1, bias = False)
        self.conv4 = nn.Conv1d(input_channels, output_channels , 1, stride, bias = False)
...
class RAM1D(nn.Module):
    def __init__(
        ...
        self.attention1 = AttentionModule_stage1(64,64)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0])
        self.attention2 = AttentionModule_stage2(128,128)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1])
        self.attention3 = AttentionModule_stage3(256,256)
